...
def _communicate_shapes(tensor_send_next, tensor_send_prev, recv_prev,
                        recv_next, tensor_ndims, config):
    ...
    recv_prev_shape = [0] * tensor_ndims
    if recv_prev_shape_tensor is not None:
        recv_prev_shape = recv_prev_shape_tensor.tolist()

    recv_next_shape = [0] * tensor_ndims
    if recv_next_shape_tensor is not None:
...
def _batched_p2p_ops(*, tensor_send_prev: Optional[torch.Tensor],
...
def _p2p_ops(*, tensor_send_prev: Optional[torch.Tensor],
             tensor_recv_prev: Optional[torch.Tensor],
             tensor_send_next: Optional[torch.Tensor],
             tensor_recv_next: Optional[torch.Tensor],
             group: torch.distributed.ProcessGroup):
...
def _communicate(
        *,
        ...
        tensor_ndims: Optional[int] = None,
...
def recv_forward(
    ...
    if parallel_state.is_pipeline_first_stage():
        ...
        input_tensor, _, _ = _communicate(
            ...
            config=config,
            tensor_ndims=tensor_ndims,
        )
    return input_tensor


def recv_backward(tensor_shape: Shape,
                  config: PipelineParallelConfig) -> torch.Tensor:
...
def send_backward(input_tensor_grad: torch.Tensor,
                  config: PipelineParallelConfig) -> None:
